def validate_permutation(permutation):
    # Check if the permutation array has 512 elements
    if len(permutation) != 512:
        return False, "Array length is not 512."

    first_half = permutation[:256]
    second_half = permutation[256:]

    # Check if the first half contains all numbers from 0 to 255
    if sorted(first_half) != list(range(256)):
        return False, "First half of the array does not contain a sequence of numbers from 0 to 255."

    # Check if the second half is the same as the first half
    if first_half != second_half:
        return False, "Second half of the array is not the same as the first half."

    return True, "Permutation array is valid."

# Example usage
# Replace 'your_permutation_array' with your actual array
your_permutation_array = [151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37,
...
185, 112, 104, 218, 246, 97, 228, 251, 34, 242, 193, 238, 210, 144, 12, 191, 179, 162, 241, 81, 51, 145,
235, 249, 14, 239, 107, 49, 192, 214, 31, 181, 199, 106, 157, 184, 84, 204, 176, 115, 121, 50, 45, 127, 4,
150, 254, 138, 236, 205, 93, 222, 114, 67, 29, 24, 72, 243, 141, 128, 195, 78, 66, 215, 61, 156, 180]  # Insert your permutation array here
is_valid, message = validate_permutation(your_permutation_array)
print(message)